...
class ImportCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($data as $item) {
            ...
            list ($sexeL, $familleId, $unite, $fnName, $totem, $nom, $prenom, $naissance, $avs, $adresse, $npa, $ville, $x1, $x2, $x3, $groupe, $telParents, $telGars, $mailParents, $emailPerso, $entree) = $item;
            ...
            $sexe = in_array(strtolower($sexeL), ['h', 'm']) ? Personne::HOMME : Personne::FEMME;
            ...
            if ($familleId && isset($poolFamilles[$familleId])) {
                ...
                if ($familleId) {
                    $poolFamilles[$familleId] = $famille;
                }
            }

            $membre = new TDGLMembre();
            $membre->setNumeroAvs($avs);
            ...
            if ($emailPerso)
                $membre->addEmail(new Email($emailPerso));
            if ($totem) $membre->setTotem($totem);
            if ($telGars)
